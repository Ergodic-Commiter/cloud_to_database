...
### Estos comandos son de referencia.  
# Úsense bajo su propio riesgo. 
...
docker run --platform=linux/amd64 --rm -it --name myfunc -p 8080:80 \ 
  ...
  $IMG:$TAG
# Antes -p 7071:80, then tried 8080:80

# Algunos Docker-ACR necesitan permisos raros.  ❓
docker tag "$IMG:$TAG" "$ACR.azurecr.io:1.0"

docker buildx build --platform linux/amd64 -t "$ACR_LOGIN/$IMG" .
docker push "$FQ_IMG"

# A veces no funciona docker, y hay que hacer Login
...
#### Desde la FunctionAPP 

az functionapp config container set -g $RG -n $FUNC \
  --image $FQ_IMG --registry-server https://$ACR.azurecr.io
...
## Plan & App  # Este parece que sólo una vez. 
